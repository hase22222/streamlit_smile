...
if camera_img is not None:
    ...
    if not results.multi_face_landmarks:
        st.write('顔全体が正面に収まるように再撮影を行ってください↑↑')
    else:
        ...
        # 目と鼻のY座標がほぼ同じか判定
        if abs(left_eye_y - right_eye_y) <= 5.0 and abs(nose_tip_y - ((left_eye_y + right_eye_y) / 2)) > 30:
            height, width, _ = img.shape  # 画像の寸法を取得
        ...
        # ①：右口角と左口角の位置が、上唇より上昇している
            ...
            if state == '素晴らしい笑顔ですね！！':
                st.download_button(label = 'ダウンロード', data = BufferedReader_img, file_name = "Duchenne_Smile.png", mime = "image/png")
            else:
                st.download_button(label = 'ダウンロード', data = BufferedReader_img, file_name = "NonDuchenne_Smile.png", mime = "image/png")
        else:
            state = ("顔が正面を向いていません。再撮影を行ってください↑↑")
            st.markdown('<style>h4{font-size: 20px;}</style>', unsafe_allow_html=True)
            st.markdown(f'<h4>{state}</h4>', unsafe_allow_html=True)
